...
class BaseQueryBuilder(object):
    ...
    def dive(self, model, path, result: list = None):
        ...
        if field in list(map(lambda col: col.key, simple_attrs + fk_attrs)):
            ...
            for relation_object in many_to_many_rels:
                if field == relation_object.key:
                    next_model, through_table = _lookup_model_manytomany_rel(relation_object).values()
                    is_many_to_many = True
                    break

        if next_model is None or next_model == []:
            if path:  # If we didn't reached the last element of the path without any model, it's an error
        ...
        if is_many_to_many:
            result.extend([
                {'model': model, 'local_join_column': None, 'remote_join_column': None}
